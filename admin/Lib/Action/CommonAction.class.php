...
//			echo $model->getlastsql();
	...
	protected function _userlist($model, $map, $type='',$sortBy = '', $asc = false) {
		//排序字段 默认为主键名
		if (isset ( $_REQUEST ['_order'] )) {
			$order = $_REQUEST ['_order'];
		} else {
		...
		if ($count > 0) {
			...
			foreach ($voList as $key =>$val){
				$user_info=D("User")->where('id = '.$val['user_id'])->field('email,mobile')->find();
				$voList[$key]['email'] = $user_info['email'];
				$voList[$key]['iphone'] = $user_info['mobile'];
				$voList[$key]['ltype'] = $type;
			}	
			//			echo $model->getlastsql();
			//分页跳转的时候保证查询条件
			foreach ( $map as $key => $val ) {
