...
class ForeAction extends CommonAction{
	...
	{	
		...
		$cate_list = M("DealCate")->where('pid=41')->findAll();
	...
	public function add()
	{
		...
		$this->assign("new_sort", M("Fore")->max("sort")+1);
		$this->display();
	}
	...
	public function edit() {		
		...
		$qa_list = M("ForeFaq")->where("fore_id=".$vo['id'])->order("sort asc")->findAll();
	...
	public function insert() {
		...
		if (false !== $list) {
			...
			{
				$deal_count = M("Fore")->where("user_id=".$data['user_id']." and is_effect = 1 and is_delete = 0")->count();
	...
	public function update() {
		...
		if (false !== $list) {
			...
			//成功提示			
			M("ForeFaq")->where("fore_id=".$data['id'])->delete();
			foreach($_REQUEST['question'] as $k=>$v)
			{
				if(trim($v)!=""||trim($_REQUEST['answer'][$k])!='')
				{
			...
			M("Fore")->where("id=".$data['id'])->setField("deal_extra_cache","");
			M("ForeLog")->where("fore_id=".$data['id'])->setField("deal_info_cache","");
			M("ForeComment")->where("fore_id=".$data['id'])->setField("deal_info_cache","");
			syn_fore($data['id']);
	...
	public function update_all(){
		$re=$GLOBALS['db']->getAll("select * from  ".DB_PREFIX."fore where  is_effect = 1 and is_delete=0 ");
		foreach($re as $k=>$v){
			syn_fore($v['id']);
	...
	public function set_sort()
	{
		...
		$log_info = M("Fore")->where("id=".$id)->getField("name");
		if(!check_sort($sort))
		{
			$this->error(l("SORT_FAILED"),1);
		}
		M("Fore")->where("id=".$id)->setField("sort",$sort);
	...
	public function foreverdelete() {
		...
		if (isset ( $id )) {
				$condition = array ('id' => array ('in', explode ( ',', $id ) ) );
				$link_condition = array ('fore_id' => array ('in', explode ( ',', $id ) ) );
				$rel_data = M(MODULE_NAME)->where($condition)->findAll();				
				foreach($rel_data as $data)
				{
					$info[] = $data['name'];	
				...
				if ($list!==false) {					
					M("ForeFaq")->where($link_condition)->delete();
					M("ForeComment")->where($link_condition)->delete();
					M("ForeFocusLog")->where($link_condition)->delete();
					M("ForeItem")->where($link_condition)->delete();
					M("ForeItemImage")->where($link_condition)->delete();
					M("ForeItemOrder")->where($link_condition)->delete();
					M("ForeSupportLog")->where($link_condition)->delete();
					M("ForeVisitLog")->where($link_condition)->delete();
					M("ForeLog")->where($link_condition)->delete();
					M("UserForeNotify")->where($link_condition)->delete();
	...
	public function deal_item()
	{
		...
		$deal_info = M("Fore")->getById($deal_id);
		$this->assign("deal_info",$deal_info);
		if($deal_info)
		{
			$map['fore_id'] = $deal_info['id'];		
			if (method_exists ( $this, '_filter' )) {
				$this->_filter ( $map );
			}
			$name=$this->getActionName();
	...
	public function insert_deal_item() {
		B('FilterString');
		$ajax = intval($_REQUEST['ajax']);
		$data = M("ForeItem")->create ();

		//开始验证有效性
		$this->assign("jumpUrl",u(MODULE_NAME."/add_deal_item",array("id"=>$data['fore_id'])));
		
		
		// 更新数据
		
		$list=M("ForeItem")->add($data);
		$log_info =  "项目ID".$data['fore_id'].":".format_price($data['price']);	
		if (false !== $list) {
			//成功提示
			...
			foreach($imgs as $k=>$v)
			{
				if($v!='')
				{
					$img_data['fore_id'] = $data['fore_id'];
					$img_data['fore_item_id'] = $list;
					$img_data['image'] = $v;
					M("ForeItemImage")->add($img_data);
				}
			}
			M("Fore")->where("id=".$data['fore_id'])->setField("deal_extra_cache","");
			save_log($log_info.L("INSERT_SUCCESS"),1);
			syn_fore($data['fore_id']);
			$this->success(L("INSERT_SUCCESS"));
		} else {
			//错误提示
			save_log($log_info.L("INSERT_FAILED"),0);
	...
	public function insert_deal_item_shouhuo(){
		...
		$data = M("ForeItemShouhuo")->create ();
		$this->assign("jumpUrl",u(MODULE_NAME."/deal_item_addshouhuo",array("id"=>$data['fore_item_id'])));
		if(!check_empty($data['kuaidi_name']))
		{
			$this->error("请输入名称");
		}
		$list=M("ForeItemShouhuo")->add($data);
		$this->success(L("INSERT_SUCCESS"));
...
public function insert_deal_xianhuo_shouhuo(){
	...
	public function edit_deal_item()
	{
		$id = intval($_REQUEST ['id']);
		$condition['id'] = $id;		
		$vo = M("ForeItem")->where($condition)->find();
		$this->assign ( 'vo', $vo );
		//输出图片集
		$img_list = M("ForeItemImage")->where("fore_item_id=".$vo['id'])->findAll();
		$imgs = array();
	...
	public function edit_deal_item_shouhuo()
	{
		$id = intval($_REQUEST ['id']);
		$condition['id'] = $id;
		$vo = M("ForeItemShouhuo")->where($condition)->find();
		$this->assign ( 'vo', $vo );
		//输出图片集
...
//		$this->assign("img_list",$imgs);
	...
	{
		$id = intval($_REQUEST ['id']);
		$vo = M("ForeItem")->getById($id);
		$this->assign ( 'vo', $vo );
		if($vo)
		{
			$map['fore_item_id'] = $vo['id'];		
			if (method_exists ( $this, '_filter' )) {
				$this->_filter ( $map );
			}
			$name=$this->getActionName();
			$model = D ("ForeItemShouhuo");
			if (! empty ( $model )) {
				$this->_list ( $model, $map );
			}
		}
	...
	public function update_deal_item() {
		...
		// 更新数据
		$this->deal_update(intval($data['fore_id']));
		$list=M("ForeItem")->save($data);
		$log_info =  "项目ID".$data['fore_id'].":".format_price($data['price']);	
		if (false !== $list) {
			...
			//成功提示
			//开始处理图片
			M("ForeItemImage")->where("fore_item_id=".$data['id'])->delete();
			...
			M("Fore")->where("id=".$data['fore_id'])->setField("deal_extra_cache","");
			M("ForeLog")->where("fore_id=".$data['fore_id'])->setField("deal_info_cache","");
			//end 处理图片
			save_log($log_info.L("UPDATE_SUCCESS"),1);
			syn_fore($data['fore_id']);
			$this->success(L("UPDATE_SUCCESS"));
	...
	public function update_deal_item_shouhuo() {
		...
		// 更新数据
		$list=M("ForeItemShouhuo")->save($data);
		if (false !== $list) {
	...
	public function del_deal_item()
	{
		...
		if (isset ( $id )) {
				$condition = array ('id' => array ('in', explode ( ',', $id ) ) );				
				$rel_data = M("ForeItem")->where($condition)->findAll();				
				foreach($rel_data as $data)
				{
					$deal_id = $data['fore_id'];
					$info[] = format_price($data['price']);	
				}
				if($info) $info = implode(",",$info);
				$info = "项目ID".$deal_id.":".$info;
				$list = M("ForeItem")->where ( $condition )->delete();				
				if ($list!==false) {					
					M("Fore")->where("id=".$deal_id)->setField("deal_extra_cache","");
					syn_fore($deal_id);
					save_log($info.l("FOREVER_DELETE_SUCCESS"),1);
					$this->success (l("FOREVER_DELETE_SUCCESS"),$ajax);
				} else {
	...
	public function del_deal_item_shouhuo()
	{
		...
		if (isset ( $id )) {
			$condition = array ('id' => array ('in', explode ( ',', $id ) ) );
			$rel_data = M("ForeItemShouhuo")->where($condition)->findAll();
			foreach($rel_data as $data)
			{
				$deal_item_id = $data['fore_item_id'];
			}
		
			$list = M("ForeItemShouhuo")->where ( $condition )->delete();
			if ($list!==false) {
				$this->success (l("FOREVER_DELETE_SUCCESS"),$ajax);
			} else {
				$this->error (l("FOREVER_DELETE_FAILED"),$ajax);
	...
	public function del_pay_log()
	{
		...
		if (isset ( $id )) {
				$condition = array ('id' => array ('in', explode ( ',', $id ) ) );				
				$rel_data = M("ForePayLog")->where($condition)->findAll();				
				foreach($rel_data as $data)
				{
					$deal_id = $data['fore_id'];
				...
				if($info) $info = implode(",",$info);
				$info = "项目ID".$deal_id.":".$info;
				$list = M("ForePayLog")->where ( $condition )->delete();				
				if ($list!==false) {					
...
$this->display("add_update");		
	...
	public function del_deal_log()
	{
		...
		if (isset ( $id )) {
				...
				$rel_data = M("ForeLog")->where($condition)->findAll();				
				foreach($rel_data as $data)
				{
					$deal_id = $data['fore_id'];
					$info[] = $data['id'];	
				...
				$list = M("ForeLog")->where ( $condition )->delete();	
							
				if ($list!==false) {		
					$GLOBALS['db']->query("update ".DB_PREFIX."fore set log_count = log_count - ".intval($list)." where id = ".$deal_id);			
					M("ForeComment")->where($condition_log)->delete();
					save_log($info.l("FOREVER_DELETE_SUCCESS"),1);
					$this->success (l("FOREVER_DELETE_SUCCESS"),$ajax);
				} else {
					save_log($info.l("FOREVER_DELETE_FAILED"),0);
	...
	public function batch_refund()
	{
		...
		$deal_info = M("Fore")->where("id=".$deal_id." and is_delete = 0 and is_effect = 1 and is_success = 0 and end_time <>0 and end_time <".get_gmtime())->find();
		if(!$deal_info)
		{
		...
		{
			require_once APP_ROOT_PATH."system/libs/user.php";
			$refund_order_list = M("ForeItemOrder")->where("fore_id=".$deal_id." and is_refund = 0 and order_status = 3")->limit($limit)->findAll();
			foreach($refund_order_list as $k=>$v)
			{
				$GLOBALS['db']->query("update ".DB_PREFIX."fore_item_order set is_refund = 1 where id = ".$v['id']);
				if($GLOBALS['db']->affected_rows()>0)
				{
					modify_account(array("money"=>$v['total_price']),$v['user_id'],$v['deal_name']."项目失败退款");					
				}
			}
			$remain = M("ForeItemOrder")->where("fore_id=".$deal_id." and is_refund = 0 and order_status = 3")->count();
			if($remain==0)
			{
				$jump_url = u("Fore/online_index");
				$this->assign("jumpUrl",$jump_url);
				M("Fore")->where("id=".$deal_info['id'])->setField("deal_extra_cache","");
				M("DealLog")->where("deal_id=".$deal_info['id'])->setField("deal_info_cache","");
				$this->success("批量退款成功");
			}
			else
			{
				$jump_url = u("Fore/batch_refund",array("id"=>$deal_id,"page"=>$page+1));
				$this->assign("jumpUrl",$jump_url);
	...
	function deal_update($deal_id){
		$deal=$GLOBALS['db']->getRow("select * from  ".DB_PREFIX."fore where id=$deal_id");
 		$now_time=get_gmtime();
