...
// 		$deal_list['list']=$deal_invest_result['list'];
	...
	{	
		...
		$sql = "select distinct(d.id) as id,d.*,max(dsl.create_time) as dtime from ".DB_PREFIX."deal as d left join ".DB_PREFIX."deal_support_log as dsl on d.id = dsl.deal_id ".
			   " where $condition and dsl.user_id = ".$home_user_info['id']." group by dsl.deal_id order by dtime desc limit ".$limit;
