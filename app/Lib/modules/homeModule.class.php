...
// 		$deal_list['list']=$deal_invest_result['list'];
	...
	{	
		...
		$sql = "select distinct(dsl.deal_id) as did,d.* from ".DB_PREFIX."deal_support_log as dsl left join ".DB_PREFIX."deal as d on d.id = dsl.deal_id ".
			   " where $condition and dsl.user_id = ".$home_user_info['id']." order by dsl.create_time desc limit ".$limit;
	
		$sql_count = "select count(distinct(dsl.deal_id)) from ".DB_PREFIX."deal_support_log as dsl left join ".DB_PREFIX."deal as d on d.id = dsl.deal_id ".
