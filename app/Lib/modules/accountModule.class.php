...
class accountModule extends BaseModule {
	...
	public function index() {
		...
		public function shichi() {
			...
			$order_list = $GLOBALS ['db']->getAll ( "select do.* from " . DB_PREFIX . "fore_item_order as do left join " . DB_PREFIX . "fore as de on de.id=do.fore_id where $condition and do.user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) . " and do.type=0 order by do.create_time desc limit " . $limit );
			foreach ( $order_list as $k => $v ) {
				if ($v ['repay_make_time'] == 0 && $v ['repay_time'] > 0) {
					$left_date = intval ( app_conf ( "REPAY_MAKE" ) ) ? 7 : intval ( app_conf ( "REPAY_MAKE" ) );
					$repay_make_date = $v ['repay_time'] + $left_date * 24 * 3600;
			...
			$order_count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "fore_item_order as do left join " . DB_PREFIX . "fore as de on de.id=do.fore_id where $condition and do.user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) . " and do.type=0" );
			...
			if ($deal_ids != null) {
				$deal_list_array = $GLOBALS ['db']->getAll ( "select * from " . DB_PREFIX . "fore where  is_effect = 1 and type=0 and is_delete = 0 and id in (" . implode ( ',', $deal_ids ) . ")" );
				$deal_list = array ();
				foreach ( $deal_list_array as $k => $v ) {
					if ($v ['id']) {
						$deal_list [$v ['id']] = $v;
	...
	public function xianhuo() {
		...
		$order_list = $GLOBALS ['db']->getAll ( "select do.* from " . DB_PREFIX . "deal_xianhuo_order as do left join " . DB_PREFIX . "deal as de on de.id=do.deal_id where $condition and do.user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) . " and do.type=0 order by do.create_time desc limit " . $limit );
		foreach ( $order_list as $k => $v ) {
			if ($v ['repay_make_time'] == 0 && $v ['repay_time'] > 0) {
				$left_date = intval ( app_conf ( "REPAY_MAKE" ) ) ? 7 : intval ( app_conf ( "REPAY_MAKE" ) );
				$repay_make_date = $v ['repay_time'] + $left_date * 24 * 3600;
		...
		$order_count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "deal_xianhuo_order as do left join " . DB_PREFIX . "deal as de on de.id=do.deal_id where $condition and do.user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) . " and do.type=0" );
		...
		if ($deal_ids != null) {
			$deal_list_array = $GLOBALS ['db']->getAll ( "select * from " . DB_PREFIX . "deal where type=0 and id in (" . implode ( ',', $deal_ids ) . ")" );
			$deal_list = array ();
			foreach ( $deal_list_array as $k => $v ) {
				if ($v ['id']) {
					$deal_list [$v ['id']] = $v;
	...
	public function do_incharge() {
		...
		showSuccess ( "", $ajax, url ( "account#pay", array (
	...
	public function go_pay() {
		...
		app_redirect ( url ( "account#jump", array (
				"id" => $notice_id 
		) ) );
	}
	...
	$GLOBALS ['tmpl']->assign ( "g_links", $g_links );
	$GLOBALS ['tmpl']->assign ( "page_title", "我的试吃列表" );
	if (! $GLOBALS ['user_info'])
		app_redirect ( url ( "user#login" ) );
	...
	$deal_list = $GLOBALS ['db']->getAll ( "select * from " . DB_PREFIX . "fore where $condition and user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) . " and is_delete = 0 order by id desc,create_time desc limit " . $limit );
	$deal_count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "fore where $condition and user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) . " and is_delete = 0" );
	$page = new Page ( $deal_count, $page_size ); // 初始化分页对象
	$p = $page->show ();
	$GLOBALS ['tmpl']->assign ( 'pages', $p );
	$GLOBALS ['tmpl']->assign ( 'deal_list', $deal_list );
		...
		if (! $order_info) {
			...
			if ($GLOBALS ['db']->affected_rows () > 0) {
				if ($money > 0) {
					...
					modify_account ( array (
					"money" => $money
					), intval ( $GLOBALS ['user_info'] ['id'] ), "删除" . $order_info ['deal_name'] . "试吃支付，退回支付款。" );
				}
			}
			showSuccess ( "", $ajax, get_gopreview () );
		}
	...
	public function view_xianhuo_order() {
		...
		if (! $GLOBALS ['user_info'])
			app_redirect ( url ( "user#login" ) );
		$GLOBALS ['tmpl']->assign ( "page_title", "购买的商品详情" );
		$id = intval ( $_REQUEST ['id'] );
	...
	public function go_order_pay() {
		...
		if (! $order_info) {
			...
			if ($result ['status'] == 0) {
				...
				app_redirect ( url ( "account#view_order", array (
						"id" => $order_info ['id'] 
				) ) );
			}
		}
	...
	public function go_shichi_order_pay() {
		...
		if (! $order_info) {
			...
			if ($result ['status'] == 0) {
				...
				app_redirect ( url ( "account#view_shichi_order", array (
				"id" => $order_info ['id']
				) ) );
			}
		}
	...
	public function go_xianhuo_order_pay() {
		...
		if (! $order_info) {
			...
			if ($result ['status'] == 0) {
				...
				app_redirect ( url ( "account#view_xianhuo_order", array (
						"id" => $order_info ['id'] 
				) ) );
			}
		}
	...
	public function chi() {
		...
		$GLOBALS ['tmpl']->assign ( "page_title", "我的试吃-报名列表" );
		$deal_id = intval ( $_REQUEST ['id'] );
		$deal_info = $GLOBALS ['db']->getRow ( "select * from " . DB_PREFIX . "fore where id = " . $deal_id . " and is_delete = 0 and is_effect = 1 and user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) );
		...
		$support_list = $GLOBALS ['db']->getAll ( "select * from " . DB_PREFIX . "fore_item_order where fore_id = " . $deal_id . " and order_status = 3 and is_refund = 0 order by create_time desc limit " . $limit );
		$support_count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "fore_item_order where fore_id = " . $deal_id . " and order_status = 3 and is_refund = 0" );
		foreach ( $support_list as $k => $v ) {
			if ($deal_info ['type'] == 1) {
				// 项目金额
				$support_list [$k] ['stock_value'] = number_format ( $deal_info ['limit_price'], 2 );
		...
		$GLOBALS ['tmpl']->display ( "account_chi.html" );
	}
	// 我的项目-购买记录
	public function goumai() {
		...
		$GLOBALS ['tmpl']->assign ( "page_title", "我的商品-购买记录" );
		$deal_id = intval ( $_REQUEST ['id'] );
		$deal_info = $GLOBALS ['db']->getRow ( "select * from " . DB_PREFIX . "deal where id = " . $deal_id . " and is_delete = 0 and user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) );
		...
		$goumai_list = $GLOBALS ['db']->getAll ( "select * from " . DB_PREFIX . "deal_xianhuo_order where deal_id = " . $deal_id . " and order_status = 3 and is_refund = 0 order by create_time desc limit " . $limit );
		$goumai_count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "deal_xianhuo_order where deal_id = " . $deal_id . " and order_status = 3 and is_refund = 0" );
		
		$GLOBALS ['tmpl']->assign ( "goumai_list", $goumai_list );
		$page = new Page ( $goumai_count, $page_size ); // 初始化分页对象
		$p = $page->show ();
		$GLOBALS ['tmpl']->assign ( 'pages', $p );
		
		$GLOBALS ['tmpl']->display ( "account_goumai.html" );
	}
	...
	public function save_chi_repay() {
		...
		$deal_info = $GLOBALS ['db']->getRow ( "select * from " . DB_PREFIX . "fore where id = " . $order_info ['fore_id'] . " and is_delete = 0 and is_effect = 1 and user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) );
		if (! $deal_info) {
			showErr ( "无权为该订单设置回报", $ajax );
		}
	...
	public function save_xianhuo_repay() {
		$ajax = intval ( $_REQUEST ['ajax'] );
		if (! $GLOBALS ['user_info']) {
			showErr ( "", $ajax, url ( "user#login" ) );
		}
		...
		$deal_info = $GLOBALS ['db']->getRow ( "select * from " . DB_PREFIX . "deal where id = " . $order_info ['deal_id'] . " and is_delete = 0 and is_effect = 1 and user_id = " . intval ( $GLOBALS ['user_info'] ['id'] ) );
		if (! $deal_info) {
			showErr ( "无权为该订单设置回报", $ajax );
		}
		...
		send_notify ( $order_info ['user_id'], "您购买的商品" . $order_info ['deal_name'] . "已发货", "account#view_xianhuo_order", "id=" . $order_info ['id'] );
		showSuccess ( "回报设置成功", $ajax );
	}
	// 我的项目
...
// 		}
	...
	public function record_go_pay() {
		...
		$return ['jump'] = url ( "account#jump", array (
				'id' => $id 
		) );
	...
	public function do_scbujiao() {
		...
		if ($GLOBALS ['user_info'] ['money'] < $money) {
			...
			$GLOBALS ['db']->query ( "update " . DB_PREFIX . "fore set pay_amount = pay_amount + " . $money . ",delivery_fee_amount = delivery_fee_amount + " . $money . " where id = " . $deal_id . " and is_effect = 1 and is_delete = 0" );
			require_once APP_ROOT_PATH . "system/libs/user.php";
			$re = modify_account ( array (
					"money" => "-" . $money
			), intval ( $GLOBALS ['user_info'] ['id'] ), "补交" . $order_info ['deal_name'] . "项目快递费用" );
		}
		showSuccess ( "", $ajax, url ( "account#shichi" ) );
	}
	public function do_sbujiao() {
		...
		if ($GLOBALS ['user_info'] ['money'] < $money) {
			...
			$re = modify_account ( array (
					"money" => "-" . $money 
			), intval ( $GLOBALS ['user_info'] ['id'] ), "补交" . $order_info ['deal_name'] . "商品快递费用" );
		}
		showSuccess ( "", $ajax, url ( "account#xianhuo" ) );
	}
