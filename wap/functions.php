...
function init_fore_page($deal_info) {
	$root ['page_title'] = $deal_info ['name'];
	if ($deal_info ['seo_title'] != "")
		$root ['seo_title'] = $deal_info ['seo_title'];
	if ($deal_info ['seo_keyword'] != "")
...
function init_deal_page_wap($deal_info) {
	...
	$deal_info ['deal_xianhuo_count'] = 0;
	foreach ( $deal_info ['deal_xianhuo_list'] as $k => $v ) {
		// 统计所有真实+虚拟（钱）
		// 统计每个子项目真实+虚拟（钱）
		$deal_info ['deal_xianhuo_list'] [$k] ['person'] = $v ['virtual_person'] + $v ['support_count'];
		$deal_info ['deal_xianhuo_list'] [$k] ['money'] = $v ['price'] * $v ['virtual_person'] + $v ['support_amount'];
		$deal_info ['deal_xianhuo_list'] [$k] ['cart_url'] = url_wap ( "cart#index", array (
				"id" => $v ['id'],
				"type" => 2 
		) );
		if ($v ['limit_user']) {
			$deal_info ['deal_xianhuo_list'] [$k] ['remain_person'] = $v ['limit_user'] - $v ['virtual_person'] - $v ['support_count'];
		}
		$deal_info ['deal_xianhuo_count'] ++;
	}
...
function init_fore_page_wap($deal_info) {
	$GLOBALS ['tmpl']->assign ( "page_title", $deal_info ['name'] );
	if ($deal_info ['seo_title'] != "")
		$GLOBALS ['tmpl']->assign ( "seo_title", $deal_info ['seo_title'] );
	if ($deal_info ['seo_keyword'] != "")
	...
	$deal_info ['comment_url'] = url_wap ( "fore#comment", array (
			"id" => $deal_info ['id'] 
	) );
	$deal_info ['info_url'] = url_wap ( "fore#info", array (
			"id" => $deal_info ['id'] 
	) );
